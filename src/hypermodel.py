...
class HyperGATConv(torch.nn.Module):
    ...
    def forward(self, node_feat: Tensor, pin_feat: Tensor, edge_index: Tensor, edge_attr: Tensor):
        ...
        pin_feat = self.e2p(
            x=(edge_feat, pin_feat),
            edge_index=edge_pin,
            size=(edge_attr.shape[0], pin_feat.shape[0]),
        )
...
class EGNNet(torch.nn.Module):
    ...
    def forward(
        ...
        for i, conv in enumerate(self.convs):
            zipped = list(
                map(
                    lambda p: conv(p[0], p[1], num_nearest=128 if p[0].shape[1] > 128 else 0),
                    zipped,
                )
            )
...
class HGNN(torch.nn.Module):
    ...
    def __init__(self, args=None):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.SiLU(negative_slope=self.negative_slope, inplace=True),

...
from src.util import mean_dist, position_encoding, draw_rect, get_density_map
...
class PlainClusterSet(Dataset):
    ...
    def process(self):
        ...
        for design in self.raw_file_names:
            ...
            for name in tqdm(names):
                ...
                # density map
                pic = get_density_map(macro_index,self.num_bins,self.bin_size,node_pos, cell_size, edge_index, pins, B)
    ...
    def pre_load_data(self, idx):
        ...
        # AP modify here 
        placement_path = osp.join(self.data_path, "raw", design, "placement_graph", str(idx - self.ptr[design]) + ".jpg")
        image = Image.open(placement_path).convert("RGB")
        ...
        if self.mode == 'HGNN':
            ...
            bipartdata = BipartiteData(x=x, edge_index=netlist.edge_index, y=y, pic = image, \
            ...
            w = self.weight[design].view(1, -1)
            bipartdata = Data(y=y, density = image, design = design, w=w)

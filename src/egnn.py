...
class EGNN(nn.Module):
    def __init__(
        ...
        edge_input_dim = (
            (fourier_features * 2) + (dim * 2) + edge_dim + 1 + 4 * use_rel_coord
        )
    ...
    def forward(self, feats, coors, edges=None, mask=None, adj_mat=None, num_nearest=0):
        (
            b,
            n,
            d,
...
class EGNN_DENSE(nn.Module):
    def __init__(
        ...
        self.use_rel_coord = use_rel_coord
        edge_input_dim = (
            (fourier_features * 2) + (dim * 2) + edge_dim + 1 + 4 * self.use_rel_coord
        )
        dropout = nn.Dropout(dropout, inplace=True) if dropout > 0 else nn.Identity()

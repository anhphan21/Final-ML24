...
def get_density_map(macro_index, num_bins, bin_size, node_pos, cell_size, edge_index, pins, B):
    density = []
    ...
    pin_density = pin_density + scatter(all_pin_pos.new_ones(pin_pos.shape[0]), indx, dim=0, dim_size=num_bins * num_bins, reduce="sum")

    cnt_density = cnt_density + scatter(B[pin_mask], indx, dim=0, dim_size=num_bins * num_bins, reduce="sum")
